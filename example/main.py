...
TODO:  
    - Add status bar
    - Clean up the code (break the code in small files)
    - Implement the 3d engine
    - Fix the text editor background color
"""
...
class AppState():
    ...
    def __init__(self):  
        ...
        self.imoguizmo_camera_version: bool = True
    ...
    while not glfw.window_should_close(window):
        ...
        if imgui.begin(f'{icons_fontawesome.ICON_FA_CUBES} 3D Viewport'):
            ...
            if not  app_state.imoguizmo_camera_version:
                ...
                is_view_changed,  new_view_matrix, is_gizmo_hovering,  is_imoguizmo_dragging = PyImOGuizmo.draw_gizmo(viewport_camera.get_view_matrix(), 10)
        ...
        if imgui.begin(f'{icons_fontawesome.ICON_FA_SYNC} PyImOGuizmos Options' ):
            ...
            if (imgui.radio_button("Camera Version", app_state.imoguizmo_camera_version == True)):
                app_state.imoguizmo_camera_version = True
            imgui.same_line()
            if(imgui.radio_button("View Matrix Version", app_state.imoguizmo_camera_version == False)):
                app_state.imoguizmo_camera_version = False
        ...
        if imgui.begin(f'{icons_fontawesome.ICON_FA_SLIDERS_H} Property Inspector' ):
            ...
            _, viewport_camera.FOV = imgui.drag_float("FOV##Camera", viewport_camera.FOV)
            
            pos_changed, new_pos= imgui.drag_float3("Position##Camera", viewport_camera.position, v_speed=0.01)
            if pos_changed:
                viewport_camera.position = glm.vec3(new_pos)
                
            changed, new_pitch  = imgui.drag_float("Pitch", viewport_camera.pitch,  v_speed=0.1, format="%.1f deg")
            if changed:
                viewport_camera.pitch = new_pitch
                viewport_camera.update_camera_vectors()
            
            changed, new_yaw  = imgui.drag_float("Yaw", viewport_camera.yaw,  v_speed=0.1, format="%.1f deg")
            if changed:
